...
(def-state-method get-for-nexts ((state ng-loop-state))
  ...
  (if-let (end (first (member-if (rcurry #'@typep 'end-state) (@state-nexts state))))
    (list end)
    (@state-nexts state)))
            ...
            (let ((nexts (mappend (lambda (n)
                  ...
                  (id (if-let (id (first (member-if (lambda (s) (cond-equal cond (@state-cond s)))
                        ...
                        (let ((id (gensym)))
                          ...
                          id))))
              (let ((end (first (member-if (rcurry #'@typep 'end-state) nexts)))) 
                (if (and end in-loop?)
...
(def-state-method add-to-new-normalized-rule ((state state) &optional in-loop?)
  (unless (visit state)
    (let* ((state-nexts-r (get-nexts state))
           (end-state (first (member-if (lambda (s) (@typep s 'end-state)) state-nexts-r)))
